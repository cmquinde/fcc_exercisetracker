...
let uri ='mongodb+srv://movildesa:414vapHh1kdg9ACw@cluster0.ui7nr.mongodb.net/myFirstDatabase?retryWrites=true&w=majority'
...
app.post('/api/users/:_id/exercises', bodyParser.urlencoded({ extended: true }) , (request, response) => {
  ...
  User.findById(request.params._id, (error, result) => {
    if(!error){
      ...
      for (i=0; i>=result.log.length; i++){
        responseObject.log[i].date = new 
 Date(responseObject.log[i].date).toDateString();
      }
